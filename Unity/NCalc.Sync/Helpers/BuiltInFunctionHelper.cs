...
namespace NCalc.Helpers
{
    public static class BuiltInFunctionHelper
    {
        ...
        {
            bool caseInsensitive = context.Options.HasFlag(ExpressionOptions.IgnoreCaseAtBuiltInFunctions);
            ...
            if (functionName.Equals("Round", comparison))
            {
                ...
                MidpointRounding rounding = context.Options.HasFlag(ExpressionOptions.RoundAwayFromZero)
            ...
            if (functionName.Equals("ifs", comparison))
            {
                ...
                foreach (Expression? argument in arguments.Where((_, i) => i % 2 == 0))
                {
            ...
            if (functionName.Equals("in", comparison))
            {
                ...
                object? parameter = arguments[0].Evaluate();
                var evaluation = false;
                for (var i = 1; i < arguments.Length; i++)
                {
                    if (TypeHelper.CompareUsingMostPreciseType(parameter, arguments[i].Evaluate(), context) != 0)
                    {
                        continue;
                    }
