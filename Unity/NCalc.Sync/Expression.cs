...
namespace NCalc
{
    ...
    /// It supports caching, custom parameter and function evaluation, and options for handling null parameters and iterating
    /// over parameter collections.
    /// The class manages the parsing, validation, and evaluation of expressions, and provides mechanisms for error detection
    /// and reporting.
    /// </summary>
    public partial class Expression : ExpressionBase<ExpressionContext>
    {
        ...
        public object? Evaluate()
        {
            ...
            EvaluationVisitor? evaluationVisitor = EvaluationVisitorFactory.Create(Context);
        ...
        private object? IterateParameters()
        {
            Dictionary<string, IEnumerator> parameterEnumerators =
                ParametersHelper.GetEnumerators(Parameters, out int? size);
            ...
            {
                foreach (KeyValuePair<string, IEnumerator> kvp in parameterEnumerators)
                {
                    kvp.Value.MoveNext();
